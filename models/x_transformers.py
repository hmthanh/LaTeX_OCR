...
class DynamicPositionBias(nn.Module):
    ...
    def forward(self, qk_dots):
        ...
        indices = rearrange(seq_arange, 'i -> i 1') - \
            rearrange(context_arange, 'j -> 1 j')
        indices += (n - 1)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_out = nn.Sequential(
            nn.Linear(v_dim, dim * 2), nn.GLU()) if on_attn else nn.Linear(v_dim, dim)
    ...
    def forward(
        ...
        b, n, _, h, talking_heads, collab_heads, head_scale, scale, device, has_context = * \
            x.shape, self.heads, self.talking_heads, self.collab_heads, self.head_scale, self.scale, x.device, exists(
                context)
        ...
        **kwargs
        ...
        if rel_pos_bias:
            ...
            self.rel_pos = DynamicPositionBias(
                dim=dim // 4, heads=heads, log_distance=dynamic_pos_bias_log_distance, depth=dynamic_pos_bias_mlp_depth, norm=dynamic_pos_bias_norm)
        elif alibi_pos_bias:
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            residual = residual_fn(
                dim, scale_residual=scale_residual, scale_residual_constant=scale_residual_constant)
...
class XTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        enc_transformer_kwargs['emb_dropout'] = enc_kwargs.pop(
            'emb_dropout', 0)
        ...
        dec_transformer_kwargs['emb_dropout'] = dec_kwargs.pop(
            'emb_dropout', 0)
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None, src_attn_mask=None):
        enc = self.encoder(src, mask=src_mask,
                           attn_mask=src_attn_mask, return_embeddings=True)
