...
from models.layers.create_act import create_act_layer
...
class FilterResponseNormTlu2d(nn.Module):
    def __init__(self, num_features, apply_act=True, eps=1e-5, rms=True, **_):
        ...
        self.tau = nn.Parameter(torch.zeros(
            num_features)) if apply_act else None
    ...
    def forward(self, x):
        ...
        x = x * self.weight.view(v_shape).to(dtype=x_dtype) + \
            self.bias.view(v_shape).to(dtype=x_dtype)
