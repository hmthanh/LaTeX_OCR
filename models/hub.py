...
def hf_split(hf_id):
    ...
    assert 0 < len(
        rev_split) <= 2, 'hf_hub id should only contain one @ character to identify revision.'
...
def load_model_config_from_hf(model_id: str):
    ...
    # insert hf_hub id for pretrained weight load during model creation
...
def push_to_hf_hub(
    ...
    if repo_namespace_or_url:
        repo_owner, repo_name = repo_namespace_or_url.rstrip(
            '/').split('/')[-2:]
    else:
